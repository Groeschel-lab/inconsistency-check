...
PACKAGE_URI = "https://github.com/groeschel-lab/inconsistency-check/releases/download/v0.2.0/app.zip"
...
class DeploymentContractTest(unittest.TestCase):
    ...
    def test_package_uri_targets_publication_repository(self) -> None:
        default = self.template["parameters"]["packageUri"]["defaultValue"]
        self.assertEqual(default, PACKAGE_URI)
        # A mutable "latest" URL would make a tagged deployment irreproducible.
    ...
    def test_wizard_collects_institution_name(self) -> None:
        ...
        self.assertEqual(
            self.form["view"]["outputs"]["parameters"]["institutionName"],
            "[steps('basics').institutionName]",
        )

    def test_form_outputs_sit_directly_under_view(self) -> None:
        # The portal silently falls back to the generic form if outputs is nested in properties.
        self.assertIn("outputs", self.form["view"])
        self.assertNotIn("outputs", self.form["view"]["properties"])
    ...
    def test_wizard_defaults_to_sign_in(self) -> None:
        ...
        # The portal rejects a defaultValue that is not one of the allowedValues labels.
        self.assertEqual(mode["defaultValue"], labels["entra"])
        self.assertIn("recommended", labels["entra"].lower())
        self.assertIn("allow-list", labels["none"].lower())
